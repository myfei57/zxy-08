...
func (e *Evictor) Run(key string) error {
	...
	// The quota release is made durable before the key is removed from the
	// index; a failed release then leaves the key in place and the capacity
	// intact, so a later eviction can retry instead of leaking capacity.
